...
void SamplingNoServo::SamplingClass::period(unsigned long microseconds) {
    ...
    // Nothing much to do here, just call the private setting method. IntervalTimer is initalized with the periode when it is started.
    setSamplingPeriod(microseconds);
...
bool SamplingNoServo::SamplingClass::setSamplingPeriod(unsigned long microseconds) {
  ...
  #elif defined(TEENSYDUINO)
    // Start InvervalTimer:
    Timer.begin(this->interruptCallback, microseconds);
    // Serial.println("StartedTimer!");
  #else
    #error "Architecture not supported."
  #endif
...
void SamplingNoServo::SamplingClass::interrupt(p_to_void_func interruptCallback) {
  ...
  //Serial.println("Modified Interrupt!");
}
